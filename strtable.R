...
#' https://gist.github.com/jbryer/4a0a5ab9fe7e1cf3be0e
#' From jbryer, url above, with minor modifications
#' 
...
strtable <- function(df, n=4, width=60, 
	...
	for(i in seq_along(df)) {
		if(is.factor(df[,i])) {
			...
			tab[i,]$levels <- collapse.values(levels(df[,i]), n=n.levels, width=30)
			tab[i,]$examples <- collapse.values(factor.values(df[,i]), n=n, width=20)
		} else {
